...
class Ax12:
    ...
    def set_goal_position(self, goal_pos):
        """Write goal position."""
        print("Set Goal Position of ID %s = %s" % (self.id, goal_pos))
    ...
    def set_goal_pos(self,position):
        ...
        self.set_register2(ADDR_AX_GOAL_POSITION_L,position)
    ...
    def check_error(comm_result, dxl_err):
        if comm_result != COMM_SUCCESS:
            rospy.logerr("%s" % Ax12.packetHandler.getTxRxResult(comm_result))
        elif dxl_err != 0:
            rospy.logerr("%s" % Ax12.packetHandler.getRxPacketError(dxl_err))
